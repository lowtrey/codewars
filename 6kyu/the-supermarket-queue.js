...
function queueTime(customers, n) {
  ...
  if(customers.length === 0) {
    ...
    return customers.reduce((a, b) => a + b);
      ...
      let arrayLength = customers.length < n ? customers.length : n;
      let countArray = Array(arrayLength).fill(0);
      for(let i = 0; i < customers.length; i++) {
        if(i < n) {
          countArray[i] += customers[i];
        } else {
          let smallestNum = Math.min(...countArray);
          let smallestNumIndex = countArray.indexOf(smallestNum);
          countArray[smallestNumIndex] += customers[i];
        };
      };
