...
//   letter individually). The zero key handles spaces, with one press of the key producing a space and two presses producing a zero.
...
function presses(phrase) {
  const keys = [" adgjmptw1", "behknqux0", "cfilorvy", "sz234568", "79"];
  let count = 0;
  for (let phraseIndex = 0; phraseIndex < phrase.length; phraseIndex++) {
    let currentChar = phrase[phraseIndex];
    for (let keyIndex = 0; keyIndex < keys.length; keyIndex++) {
      let currentKey = keys[keyIndex];
      if (currentKey.includes(currentChar.toLowerCase())) {
        count = count + (keyIndex + 1);
      }
    }
  }
  return count;
